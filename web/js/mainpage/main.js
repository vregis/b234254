    ...
    function fn_menu() {
        ...
        $menuToggle.on('click', function(e) {
            ...
            if ($body.hasClass('menu-in')) {
                $('.hd').hide();
                $('.ion-ios-email').show();
                $('#site-main, .header-brand, #site-footer, .sign-in').velocity('stop').velocity('fadeOut', {
                    duration: 500,
                    easing: 'easeOutQuart',
                    queue: false
                });
                ...
                $('.hd').show();
                $('.ion-ios-email').hide();
                $('#site-main, .header-brand, #site-footer, .sign-in').velocity('stop').velocity('fadeIn', {
                    duration: 800,
                    easing: 'easeInQuart',
                    queue: false
                });
  ...
  function fn_core() {
    ...
    $('head').append(
      '<style id="js-css">' +
       ...
       '.light #login { background-color: ' + _lightFormOverlayColor + '; }' +
       '.dark #overlay { background-color: ' + _darkOverlayColor + '; }' +
       '.dark #form { background-color: ' + _darkFormOverlayColor + '; }' +
       '.dark #login { background-color: ' + _darkFormOverlayColor + '; }' +
      '</style>'
    );
...
=================================================*/
  ...
  function fn_slider() {
    ...
    for (var i = 1; i <= _imgAmount; i++) {
      $('#img').append('<img src="/images/mainpage/bg/slideshow-' + (i < 10 ? '0' + i : i) + '.jpg">');
    }
...
=================================================*/
  function fn_videoBg() {
    ...
    if (!isMobile) {
      $video.append('<video id="video-bg" autoplay loop><source src="/video/video.mp4" type="video/mp4"></video>');
      if (_videoMute) {
        var video = document.getElementById('video-bg');
        video.muted = true;
      }
  ...
  function fn_volumeOn() {
    ...
    function fn_validatelogin() {
        ...
        $form.validate({
            ...
            submitHandler: function(form) {
                $(form).ajaxSubmit({
                    url: '/user/security/ajaxlogin',
                    type: 'post',
                    dataType: 'json',
                    data: {email:$('input#loginform-email').val(), password:$('input#loginform-password').val(), rememberMe:$('input#loginform-rememberme').val()},
                    success: function(data) {
                        if (data.error == false) {
                            $form.validate().resetForm();
                            $form.find('.form-control').removeClass('valid error');
        ...
        $.ajax({
            ...
            success: function(response){
                if(response.error == false){
                    ...
                    $(this).append('<div class="notify"><p class="notify-message"><i class="notify-icon"></i>gdfgsd</p></div>');
                }
            }
        })
    })*/
