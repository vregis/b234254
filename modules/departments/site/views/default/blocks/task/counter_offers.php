...
<table style="width:100%;" class="table with-foot">
    ...
    <? foreach($counter_offers as $counter_offer) : ?>
        <tr class="counter-offer-row">
            <td style="width:50px;"><img  onError="this.onerror=null;this.src='/images/avatar/nophoto.png';" class="active gant_avatar" src="<?php echo $counter_offer->delegate_avatar ? $folder_assets = Yii::$app->params['staticDomain'] .'avatars/'.$counter_offer->delegate_avatar:'/images/avatar/nophoto.png'?>"></td>
            <td style="width:160px;"><?= $counter_offer->name ?></td>
            <td style="width: 165px;"><?= getData($counter_offer->start) ?> - <?= getData($counter_offer->end) ?></td>
            ...
            <td style="width: 125px;" class="<? if($counter_offer->counter_price > $counter_offer->price) echo 'bg-red-pink bg-font-red-pink';
            ...
            <td style="width: 375px;"><button style="display:inline-block;" class="btn btn-primary confirn" data-status="0" data-delegate_task_id="<?= $counter_offer->id ?>">Reject</button>
