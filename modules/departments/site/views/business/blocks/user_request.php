...
<? foreach($users as $user) : ?>
    <tr class="user-row" data-page-id="<?= intval($i/5) ?>" style="<?=intval($i/5)!=0 ? 'display: none':'' ?>">
        ...
        consequat.</p><div class="clearfix"></div> <a target="_blank" href="/user/social/shared-profile?id=<?php echo $user->uid?>" class="btn btn-primary pull-left">My profile</a><a target="_blank" href="/departments/business/shared-business?id=<?php echo $user->tool?>" class="btn btn-primary pull-right">My business</a> <div class="clearfix"></div>'  class="active gant_avatar" src="<?php echo $user->user_avatar ? $folder_assets = Yii::$app->params['staticDomain'] .'avatars/'./*$user->user_avatar*/'':'/images/avatar/nophoto.png'?>"></td>
