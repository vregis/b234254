...
<? foreach($users as $user) : ?>
    <tr class="user-row" data-page-id="<?= intval($i/5) ?>" style="<?=intval($i/5)!=0 ? 'display: none':'' ?>">
        ...
        consequat.</p><div class="clearfix"></div> <a href="#" class="btn btn-primary pull-left">My profile</a><a href="/departments/business" class="btn btn-primary pull-right">My business</a> <div class="clearfix"></div>' class="active gant_avatar" src="<?php echo $user->ava ? $folder_assets = Yii::$app->params['staticDomain'] .'avatars/'.$user->ava:'/images/avatar/nophoto.png'?>"></td>
